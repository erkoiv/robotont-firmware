...
//Init power measurement variables
AnalogIn ain(A0), ain2(A1); //pins
uint16_t readA, readV; //ADC values
...
int main()
{
  ...
  while (true)
  {
    ...
    // Parse ADC values into (cur)rent draw and (bat)tery voltage 
    float cur = readCurrent(readA);
    float bat = batteryVoltage(readV);

    // Send power packet
    serial_pc.printf("POWER:%f:%f\r\n", cur, bat);
